...
import { parseExportSvgMetadata } from "./exportSvg"
...
export const ExcalidrawPage: QuartzPageTypePlugin<ExcalidrawPageOptions> = (opts) => ({
  ...
  generate({ ctx }) {
    ...
    for (const filePath of excalidrawFiles) {
      ...
      const exportBase = filePath.replace(/\.md$/, "")
      const lightExportPath = `${exportBase}.light.svg`
      const darkExportPath = `${exportBase}.dark.svg`
      const staticExportRoot = join(process.cwd(), "quartz", "static", "excalidraw")
      const hasLightExport = existsSync(join(staticExportRoot, lightExportPath))
      const hasDarkExport = existsSync(join(staticExportRoot, darkExportPath))
      const metadataPath = hasLightExport
        ? lightExportPath
        : hasDarkExport
          ? darkExportPath
          : undefined
      ...
      if (metadataPath) {
        try {
          exportMetadata = parseExportSvgMetadata(
            readFileSync(join(staticExportRoot, metadataPath), "utf-8"),
          )
        } catch {
      ...
      virtualPages.push({
        ...
        data: {
          ...
          excalidrawExport:
            exportMetadata && (hasLightExport || hasDarkExport)
              ? {
                  lightPath: hasLightExport ? `/static/excalidraw/${lightExportPath}` : undefined,
                  darkPath: hasDarkExport ? `/static/excalidraw/${darkExportPath}` : undefined,
                  ...exportMetadata,
                }
              : undefined,
        },
