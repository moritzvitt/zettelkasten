...
export interface RenderContext {
  ...
  resolvedNoteImages?: Record<string, ResolvedNoteImage>;
}
...
export function renderToSvg(
  ...
  for (const el of overlayElements) {
    const embeddedTarget = el.fileId ? data.embeddedFiles?.[el.fileId] : undefined;
    const link = embeddedTarget ? `[[${embeddedTarget}]]` : ((el.link as string) ?? "");
...
function renderImage(el: ExcalidrawElement, data: ExcalidrawData, ctx?: RenderContext): string {
  ...
  const noteImage = ctx?.resolvedNoteImages?.[el.id];
  if (noteImage) {
    const href = escapeAttr(noteImage.href);
    const title = escapeAttr(noteImage.title);
    return `<a href="${href}" data-note-href="${href}" data-note-title="${title}" aria-label="${title}"><image x="${el.x}" y="${el.y}" width="${el.width}" height="${el.height}" href="${escapeAttr(noteImage.dataUrl)}" preserveAspectRatio="xMidYMid meet" /></a>`;
  }
