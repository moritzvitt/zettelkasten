...
describe("renderToSvg", () => {
  ...
  it("renders note snapshots as linked SVG images", () => {
    const result = renderToSvg(
      {
        type: "excalidraw",
        version: 2,
        appState: {},
        files: {},
        embeddedFiles: { "note-file": "Notes/My note" },
        elements: [
          {
            id: "note-image",
            type: "image",
      ...
      {},
      {
        resolvedNoteImages: {
          "note-image": {
            dataUrl: "data:image/svg+xml;charset=utf-8,note-preview",
            href: "/my-note",
            title: "My note",
          },
        },
    ...
    expect(result.svg).toContain('<a href="/my-note"');
    expect(result.svg).toContain('data-note-href="/my-note"');
    expect(result.svg).toContain("<image");
