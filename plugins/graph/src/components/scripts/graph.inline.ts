...
(function () {
  ...
  function initGraph() {
    ...
    function getStoredDepth() {
      var value = Number(localStorage.getItem(depthStorageKey));
      return allowedDepths.indexOf(value) === -1 ? null : value;
    }

    function applyStoredDepth(container) {
      var storedDepth = getStoredDepth();
      if (storedDepth === null) return;
      var config = JSON.parse(container.dataset["cfg"] || "{}");
      config.depth = storedDepth;
      container.dataset["cfg"] = JSON.stringify(config);
    }

    function updateDepthButtons(depth) {
      var buttons = document.querySelectorAll(".graph-depth-control button");
      for (var i = 0; i < buttons.length; i++) {
        var isActive = Number(buttons[i].dataset["depth"]) === depth;
    ...
    function handleNav(e) {
      ...
      if (depthClickHandler) {
        for (var i = 0; i < depthButtons.length; i++) {
          depthButtons[i].removeEventListener("click", depthClickHandler);
        }
      }

      depthButtons = Array.from(document.querySelectorAll(".graph-depth-control button"));
      depthClickHandler = function (event) {
        var depth = Number(event.currentTarget.dataset["depth"]);
        if (allowedDepths.indexOf(depth) === -1) return;
        localStorage.setItem(depthStorageKey, String(depth));
        updateDepthButtons(depth);
        renderLocal();
      };
      for (var i = 0; i < depthButtons.length; i++) {
        depthButtons[i].addEventListener("click", depthClickHandler);
      }

      var storedDepth = getStoredDepth();
      if (storedDepth !== null) updateDepthButtons(storedDepth);
